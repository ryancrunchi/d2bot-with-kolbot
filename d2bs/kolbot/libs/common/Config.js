...
var Config = {
	init: function (notify) {
				...
				for (n in CustomConfig) {
					if (CustomConfig.hasOwnProperty(n)) {
						if (CustomConfig[n].indexOf(me.profile) > -1) {
							if (notify) {
								print(ColorCodes.NEON_GREEN + "Loading custom config: " + ColorCodes.YELLOW + n + ".js");
							}
		...
		if (FileTools.exists("libs/config/" + configFilename)) {
			...
			if (notify) {
				print(ColorCodes.RED + classes[me.classid] + "." + me.charname + ".js not found!"); // Use the primary format
				print(ColorCodes.RED + "Loading default config.");
			}
			...
			if (!FileTools.exists("libs/config/" + classes[me.classid] + ".js")) {
				...
				throw new Error(ColorCodes.RED + "Default config not found. \n" + ColorCodes.YELLOW + "     Try reading the kolbot wiki.");
			}
			...
			try {
				...
				throw new Error(ColorCodes.RED + "Failed to load default config.");
			}
		}
		...
		try {
			...
			if (notify) {
				print(ColorCodes.ORANGE + "Error in " + e2.fileName.substring(e2.fileName.lastIndexOf("\\") + 1, e2.fileName.length) + "(line " + e2.lineNumber + "): " + e2.message);
		...
		try {
			...
			print(ColorCodes.ORANGE + "Error in libs/common/AutoBuild.js (AutoBuild system is not active!)");
			print(e3.toSource());
		}
	},
