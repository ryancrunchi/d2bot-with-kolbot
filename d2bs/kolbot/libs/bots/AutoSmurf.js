	...
	this.partyAct = function () { // Cycles thru getParty() and returns the lowest Act (i.e., 1-5) the partied characters are in. Quits if noone is partied. Returns false is someone isn't in a Town.
		...
		for (i = 0 ; i < 30 ; i += 1) { // Try 30 times because getParty(); can fail once in a while.
			...
			if (player) {
				...
				while (player.getNext()) {
					if (myPartyID != 65535 && player.partyid === myPartyID) { // Only check characters in a Party with me.
...
*/
	...
	this.Bo = function () { //Dark-f adding
		...
		else if (Config.AutoSmurf.TeamSize > 1) {
			delay(2000);
		}
		return true;
	};
...
//STUFF
	...
	this.hireMerc = function(another) {
		...
		else if (merc.classid == 338) { // already have act2 merc
			return true;
		}
		...
		if (!this.teamQuest(1, 1)) { // Haven't cleared the Den yet.
			...
			if (me.diff === 0) { // All characters grab Cold Plains Waypoint in Normal. Only the Teleporting Sorc grabs it in Nightmare and Hell.
				...
				if(teleportingSorc) {
					...
					Pather.teleport = Config.AutoSmurf.TeamSize < 2; // not teleporting in Den
	...
	this.cain = function () { // Dark-f: rewrite rescue cain
		...
		if (!this.teamQuest(4, 1) ) { // not rescues Cain
			...
			if(teleportingSorc) {
				...
				Pather.makePortal();
				if(me.diff > 0)
					Pather.teleport = Config.AutoSmurf.TeamSize < 2;
			} else {
				Town.move("portalspot");
	...
	this.radament = function () {
		...
		if (!this.teamQuest(9, 1)) {
			if (teleportingSorc) {
				...
				while (getDistance(me.x, me.y, radament.roomx * 5 + radament.x, radament.roomy * 5 + radament.y) > 30) {
					try {
						Pather.moveToPreset(49, 2, 355, 10, 10, false, false);
					} catch (e) {
						print("Caught Error.");
						print(e);
					}
			...
			if (teleportingSorc) {
				...
				if (me.classid ===1 ) {
					...
					while(true){
						...
						anya = getUnit(2, 558);
						if( anya && getDistance(me, anya) < 15)
							break;				
					}
				}
				Pather.makePortal();
...
*/
	...
	if (!this.teamQuest(23, 0) && this.teamQuest(15, 0)) { // "Able to go to Act IV" (AKA haven't gone thru red portal to Act 4) is not done and Duriel is.
		...
		else if (teleportingSorc ) { // I am the Teleporting Sorc
