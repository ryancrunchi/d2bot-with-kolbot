...
function LoadConfig() {
	...
	// *** act 1 ***
	Scripts.Cows = false;
	Scripts.CowsHelper = false;
		Config.CowsHelper.Wait = 120; // seconds to wait for cow level portal
	...
	*/
	...
	Config.Leader = "Chibralair"; // Leader's ingame character name. Leave blank to try auto-detection (works in AutoBaal, Wakka, MFHelper)
	Config.QuitList = ["Chibralair"]; // List of character names to quit with. Example: Config.QuitList = ["MySorc", "MyDin"];
	...
	Scripts.BaalHelper = false;
		Config.BaalHelper.Wait = 240; // Seconds to wait for a runner to be in Throne
	...
	Scripts.IPHunter = false;
		Config.IPHunter.IPList = [46, 50]; // List of IPs to look for. example: [165, 201, 64]
	...
	Config.PingQuit = [{Ping: 1000, Duration: 15}]; // Quit if ping is over the given value for over the given time period in seconds.

	// Shrine Scanner - scan for shrines while moving.
	// Put the shrine types in order of priority (from highest to lowest). For a list of types, see sdk/shrines.txt
	Config.ScanShrines = [12, 2, 1, 3, 20];
	...
	Config.Redemption = [74, 40]; // Switch to Redemption after clearing an area if under designated life or mana. Format: [lifepercent, manapercent]
	
	
	// AutoBuild System ( See /d2bs/kolbot/libs/config/Builds/README.txt for instructions )
	Config.AutoBuild.Enabled = true;			//	This will enable or disable the AutoBuild system
	
	Config.AutoBuild.Template = "Hammer";	//	The name of the build associated with an existing 
	...
	Scripts.AutoSmurf = true;
		...
		Config.AutoSmurf.TeleportingSorc = "Chibralair";
		Config.AutoSmurf.BoBarb = "";
		Config.AutoSmurf.NonSorcChar = ["Chibradin"];
      	Config.AutoSmurf.AllTeamProfiles = ["ASBlizz", "ASHam"];


	//NOTE: Use PROFILE names (= window title), NOT character names
    Scripts.AutoSmurf2 = false;
        Config.AutoSmurf2.Smurfette = "ASBlizz";
            //sorc profile name, will teleport (required)
        Config.AutoSmurf2.BigSmurf = "";
            //barb profile name, will lead/bo (required)
        Config.AutoSmurf2.Smurfs = [];
