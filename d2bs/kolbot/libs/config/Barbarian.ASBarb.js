...
function LoadConfig() {
	...
	*/
	...
	Config.QuitList = ["ASFireOrb"]; // List of character names to quit with. Example: Config.QuitList = ["MySorc", "MyDin"];
	...
	/*Scripts.AutoSmurf = false; 
		...
		Config.AutoSmurf.TeleportingSorc = ["BooZ"]; // ingame character(s) name(s) - ex: ["SorcTeleportA", "SorcTeleportB", ""];
			//- search brain - teleport to diablo/baal - if you have less than 3 sorc, use this - 3 characters max
		Config.AutoSmurf.QuestingSorcA = ["BooZ"]; // ingame character(s) name(s) - ex: ["SorcFireBall", "", ""];
		...
		Config.AutoSmurf.BoBarb = BarbooZ""; // ingame character name - ex: "BarbBo";
	...
	Scripts.AutoSmurf = true;
		...
		Config.AutoSmurf.BoBarb = "BarbooZ";
		Config.AutoSmurf.NonSorcChar = [/*"Chibradin"*/];
      	Config.AutoSmurf.AllTeamProfiles = ["ASFireOrb", "ASBarb"];
}